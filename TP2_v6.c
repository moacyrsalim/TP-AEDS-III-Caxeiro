#include <stdio.h>
#include <stdlib.h>
#define P_ATUAL grafo[aresta->destino].habilidade/grafo[aresta->destino].peso
#define P_MELHOR grafo[melhor].habilidade/grafo[melhor].peso
...
void percorre_guloso(vertice* grafo, int n, int peso_max, int dist_max){
	...
	int habilidade_total, distancia_percorrida;
	melhor = 1;
	j = 0;
	for (i = 2; i < n; i++){
	...
	while(j<=n && peso_max>0){

		while(aresta != NULL){
			if((float)grafo[aresta->destino].habilidade/grafo[aresta->destino].peso > (float)grafo[melhor].habilidade/grafo[melhor].peso)
			melhor = aresta->destino;
			distancia_percorrida = aresta->distancia;
			aresta = aresta->prox;
			
		}
		if(dist_max - distancia_percorrida < 0){
			j++;
			continue;
		}
		dist_max = dist_max - distancia_percorrida;
